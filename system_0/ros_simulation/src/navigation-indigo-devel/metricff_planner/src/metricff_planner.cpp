...
#include <metricff_planner/metricff_planner.h>
#include <metricff_planner/primitive.h>
#include <metricff_planner/readinputs.h>
#include <metricff_planner/writeconstraints.h>
#include <metricff_planner/extractoutput.h>
#include <string>
#include <iostream>
#include <fstream>
#include <sstream>
...
PLUGINLIB_EXPORT_CLASS(metricff_planner::MetricFFPlanner, multi_planner_base::MultiPlanner)

namespace metricff_planner {
	MetricFFPlanner::MetricFFPlanner() : initialized_(false)
	{
		ros::NodeHandle private_nh("~");
		ROS_ERROR("MetricFFPlanner constructor");


//		string default_value = "240";
		string primitives_filename_default_value= "primitive_movements_manhattan.txt";
//		private_nh.param("metricff_planner/path_cost_tolerance", multiPathCostTolerance, 5.0);
//		private_nh.param("metricff_planner/planning_patience", z3TimeoutInSeconds, default_value);
//		private_nh.param("metricff_planner/final_optimization", final_smt_cost_optimization, false);
		private_nh.param("metricff_planner/primitives", primitives_filename, primitives_filename_default_value);
//		double testParam;
//		private_nh.setParam("metricff_planner/testParam", 1.0);
	...
	void MetricFFPlanner::initialize()
	{
		if(!initialized_){
...
//		      }
	...
	position MetricFFPlanner::metersToImplanUnits(pos_2d pos){
	  ...
	  pos_2d MetricFFPlanner::implanUnitsToMeters(position pos){
		  pos_2d p;
		  ostringstream os, os1;
		  double IMPLAN_UNIT_SIZE = map_resolution * map_scale;
		  double IMPLAN_MAP_WIDTH = map_width;
	...
	void MetricFFPlanner::createWorkspace(dimension_t dimension, pos_vec_t initposition, pos_vec_t finalposition, int iteration, float cost, workspace_t &workspace)
	{
	  unsigned int count;
	...
	void MetricFFPlanner::generatePDDLFiles(prim_vec_t primitives, pos_vec_t obstacles, vector<position_with_cost> alwaysEventuallyFreeObstacles, workspace_t workspace)
	{
	  ofstream ofp;
	  string domain_file = static_files_folder+"metricffPlannerDomain.pddl";
	  ...
	  string task_definition_file = static_files_folder+"metricffPlannerTask.pddl";
	...
	void MetricFFPlanner::generateTrajectories(prim_vec_t primitives, dimension_t dimension, pos_vec_t obstacles, pos_vec_t initpos, pos_vec_t finalpos, int max_length)
	{
	  unsigned int count;
	  ...
	  f_output = static_files_folder+"metricff_output.txt";
	  count = 2;
	  ...
	  string command =("./"+utils_files_folder+"ff -f "+static_files_folder+"metricffPlannerTask.pddl -o "+static_files_folder+"metricffPlannerDomain.pddl -s 3 > "+f_output);
	  cout<<command<<endl;
	  system(command.c_str());
	}
	
	pos_vec_t MetricFFPlanner::convertPosesIntoScaledPositions_(std::vector<ordered_pose_stamped::OrderedPoseStamped>& poses, std::map<position, ordered_pose_stamped::OrderedPoseStamped>& conversion_map)
	{
	...
	std::vector<ordered_pose_stamped::OrderedPoseStamped> MetricFFPlanner::convertScaledPositionsIntoPoses(pos_vec_t & positions)
	{
	...
	bool MetricFFPlanner::makePlan(std::vector<ordered_pose_stamped::OrderedPoseStamped>& start_poses,
	...
	{
	  ros::NodeHandle private_nh("~");
	  //private_nh.param("metricff_planner/path_cost_tolerance", multiPathCostTolerance, 13.0);
...
/*
	  ...
	  //max search length for implan. @indranil, why is this x + y?
	   ...
	   float final_cost = extractTrajectoryCostInformation(f_output);
		ofstream ofp;
	   string res_file =static_files_folder+"/costs/metricff_planner_costs.txt";
...
/*
	...
	bool MetricFFPlanner::findPosition (pos_vec_t posvec, position pos)
	{
	  unsigned int count;
